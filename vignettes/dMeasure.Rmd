...
```{r}
?dMeasure::list_billings
a$own_billings # by default is TRUE, and shows only billings attributable to the provider
...
a$list_billings() %>% head() # an aggregated version of $appointments_billings_sameday()
```
...
```{r}
a$own_billings <- FALSE
a$list_billings() %>% head()
...
a$appointment_status <- "Completed" # choose all appointment status
a$list_contact_appointments() %>>% head()
...
```{r}
?dMeasure::list_qim_active
a$contact_type <- a$contact_types # includes 'all' contact types as valid (appointment, visit, billing)
a$list_qim_active() # the list of patients, with Quality Improvement Measure specified groupings
```

The other group of methods provide aggregated/anonymized *report* of the relevant patients.

```{r}
a$report_qim_active() # an 'anonymized'/aggregated report of the active patients
...
```{r}
a$qim_demographicGroup # the default setting
a$qim_demographicGroupings
a$qim_demographicGroup <- c("Sex", "Ethnicity")
a$report_qim_active()
```
...
```{r}
a$list_qim_diabetes()
a$report_qim_diabetes()
```

By default, 'old' results (as defined by the Quality Improvement Measures) are ignored.
Old results can be included by setting **qim_ignoreOld** to *TRUE*.

```{r}
a$qim_ignoreOld <- FALSE
a$list_qim_diabetes()
a$report_qim_diabetes()
```
...
DT::datatable(a$list_qim_15plus(),
              extension = "Scroller",
              options = list(scroller = TRUE, scrollX = TRUE))
DT::datatable(a$report_qim_15plus())
```
...
DT::datatable(a$list_qim_cvdRisk() %>%
                dplyr::select(-c(RecordNo, MaritalStatus, Sexuality)),
              extension = "Scroller",
              options = list(scroller = TRUE, scrollX = TRUE))
a$report_qim_cvdRisk()
```

### Cardiovascular risk inclusion/exclusion groups

```{r}
a$qim_cvdRisk_measureTypes
a$qim_cvdRisk_measure # default group settings
```
...
```{r}
?dMeasure::framingham_riskequation
```

In the interest of openness and reproducability
the actual equation and function is reproduced below.
Please report any required corrections to me!

```{r code = capture.output(dMeasure::framingham_riskequation), eval = FALSE}
```
